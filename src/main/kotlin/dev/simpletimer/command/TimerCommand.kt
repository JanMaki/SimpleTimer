...
class TimerCommand : CommandDataImpl("timer", "タイマーを開始します。タイマーの操作は!!timerコマンドを使用してください。"), RunCommand {
    ...
    override fun runCommand(user: User, args: List<String>, message: Message) {
        ...
        if (label.equalsIgnoreCase("add")) {
            ...
            val timing = when {
                args[2].equalsIgnoreCase("LV1") -> dev.simpletimer.ServerConfig.TTSTiming.LV1
                args[2].equalsIgnoreCase("LV2") -> dev.simpletimer.ServerConfig.TTSTiming.LV2
                args[2].equalsIgnoreCase("LV3") -> dev.simpletimer.ServerConfig.TTSTiming.LV3
